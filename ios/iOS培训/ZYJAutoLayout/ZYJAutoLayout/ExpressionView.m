...
- (BOOL)pointInside:(CGPoint)point withEvent:(UIEvent *)event
{
    ...
    CGFloat width = 44 - bounds.size.width;
    CGFloat height = 44 - bounds.size.height;
...
//static NSUInteger const self.onePageCount = 8;
...
#define PageCount ((ImageCount - 1) / self.onePageCount + 1)
...
{
    _onePageCount = 8;
    self.contentSize = CGSizeMake([UIScreen mainScreen].bounds.size.width * PageCount, ViewHeight);
    self.delegate = self;
...
- (void)setupContentview
{
    ...
    [self.contentView mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.width.equalTo(self).multipliedBy(PageCount);
    }];
}
...
//    CGFloat xHalfSpace = [UIScreen mainScreen].bounds.size.width / 8.0 - ImageSize.width / 2.0;
    ...
    for (; imageViewIndex < ImageCount;) {
        ...
        for (; onePageNumber < self.onePageCount && imageViewIndex < ImageCount; onePageNumber++) {
...
//                                + [UIScreen mainScreen].bounds.size.width * (imageViewIndex / self.onePageCount);
            CGFloat topInset = 8 + (ImageSize.height + 26) * (onePageNumber / (self.onePageCount / 2));
            [imageView mas_makeConstraints:^(MASConstraintMaker *make) {
                make.size.mas_equalTo(ImageSize);
                //不能适应屏幕旋转
//                make.left.mas_equalTo(leftInset);
                //能适应屏幕旋转
                make.left.equalTo(self.contentView.mas_right).multipliedBy((1.0 / (PageCount * self.onePageCount / 2)) * (0.5 + onePageNumber % (self.onePageCount / 2)) + (1.0 / PageCount) * (imageViewIndex / self.onePageCount)).offset(-(ImageSize.width / 2.0));
...
{
    NSUInteger pageCount = (ImageCount - 1) / self.onePageCount + 1;
...
- (void)updatepageControlViewLeftConstraint:(CGFloat)leftOffset
{
    ...
    for (; pageIndex < pageCount; pageIndex++) {
        ...
        [view mas_remakeConstraints:^(MASConstraintMaker *make) {
            ...
            make.top.mas_equalTo(self).offset(ViewHeight - pageControlViewRadius - 8);
 ...
 *  @param pageNumber
 */
- (void)showPageWithNumber:(NSUInteger)pageNumber
{
...
- (void)tapPageControlView:(UIView *)view
{
    [self showPageWithNumber:view.tag];
}
...
- (NSMutableArray <UIView *> *)pageControlViewArray
{
    if (!_pageControlViewArray) {
        ...
        NSUInteger pageIndex = 0;
        for (; pageIndex < pageCount; pageIndex++) {
            UIButton *view = [[MYButton alloc] init];
            view.tag = pageIndex;
            view.layer.cornerRadius = pageControlViewRadius / 2.0;
            view.backgroundColor = [UIColor grayColor];
            [view addTarget:self action:@selector(tapPageControlView:) forControlEvents:UIControlEventTouchUpInside];
